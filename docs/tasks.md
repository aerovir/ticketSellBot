...
| 219 | Fix (errors #080): белый экран VK Mini App — jsQR грузился с внешнего CDN (jsdelivr), блокирующий скрипт перед app.js не грузился в iframe VK → app.js не стартовал (доказано прод-логами: нет /api/* запросов). Фикс: jsQR.js локально `/static/jsQR.js` в обоих entry; попутно — isVK по pathname, приоритет launch params из URL, таймаут vk-bridge, локальный vk-bridge.min.js | ✅ done | 2026-08-24 | `ab375e7` (bugfix/vk-app-external-cdn) |
