...
| 219 | Fix (errors #080): белый экран VK Mini App — `await bridge.send("VKWebAppGetLaunchParams")` зависал вне VK-окружения, launch params не извлекались из URL → initData пуст → showNoInitData(); фикс: isVK по pathname, приоритет URL-query, таймаут bridge 800мс, локальный бандл vk-bridge.min.js вместо unpkg CDN | ✅ done | 2026-08-24 | `95f1d32` (bugfix/vk-app-white-screen) |
