...
| 219 | Fix (errors #080): белый экран VK Mini App — **инлайн `style="display:none"` на онбординге** перекрывал `.onboarding-overlay.active { display:flex }` → оверлей невидим, кнопка скрыта, runAppStart() не вызывался (доказано на проде Playwright: display:none → после фикса flex, клик → API-запросы). Дополнительно: jsQR.js локально вместо CDN, vk-bridge локально, isVK по pathname, приоритет URL-params | ✅ done | 2026-08-24 | `caf8d41` (bugfix/onboarding-inline-display) |
