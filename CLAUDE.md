...
| Термин | Описание |
|--------|----------|
| **Super-admin** / **Супер-админ** | Владелец бота. Указывается через `ADMIN_TELEGRAM_IDS`. Управляет **ботом полностью** (глобально, не привязан к каналу): подписки каналов, все каналы/мероприятия, рассылка, статистика, глобальные действия. |
| **Admin** / **Организатор** | Клиент бота, создаёт мероприятия и продаёт билеты. Может иметь **свой канал** (публикует в нём, подписка на канал) **или работать без канала** (подписка на пользователя, продажи через Mini App). **Покупает подписку** (на канал или на пользователя), получает статистику, **проверяет билеты на входе**. |
| **Пользователь** | Посетитель мероприятий. **Покупает билеты**, **получает пригласительные**, на мероприятии **предъявляет код/QR на входе** (верификацию прохода и отметку входа делает организатор). |
| **Канал (Channel)** | Telegram-канал, в который добавлен бот. Предназначен для создания, отображения и хранения списка мероприятий, покупки или регистрации билетов. Мероприятие привязано к каналу ИЛИ к организатору (owner_user_id). |

> **Super-admin ≠ Организатор ≠ Пользователь.** Super-admin управляет ботом полностью. Организатор — клиент с подпиской (на канал или на пользователя), ведёт продажи и проверяет билеты. Пользователь — покупает билеты/пригласительные и предъявляет код/QR на входе.
