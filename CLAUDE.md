...
Управление мероприятиями через Telegram. Админы задаются через `ADMIN_TELEGRAM_IDS` в `.env` (Telegram ID через запятую).

### Команды админа (только в личных сообщениях)
