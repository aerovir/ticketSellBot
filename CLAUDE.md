...
| **Admin** / **Админ канала** | Клиент бота, владелец своего канала. **Покупает подписку**, чтобы вести продажи: создаёт мероприятия, продаёт билеты, получает статистику своего канала, **проверяет билеты на входе**. Управляет только своими каналами. |
| **Пользователь** | Посетитель мероприятий. **Покупает билеты**, **получает пригласительные**, на мероприятии **предъявляет код/QR на входе** (верификацию прохода и отметку входа делает админ). |
| **Канал (Channel)** | Telegram-канал, в который добавлен бот. Предназначен для создания, отображения и хранения списка мероприятий, покупки или регистрации билетов. На основе этих действий формируются QR-коды или кодовое подтверждение. |

> **Super-admin ≠ Admin ≠ Пользователь.** Super-admin управляет ботом полностью. Admin — клиент с подпиской, ведёт продажи в своём канале и проверяет билеты. Пользователь — покупает билеты/пригласительные и предъявляет код/QR на входе.
