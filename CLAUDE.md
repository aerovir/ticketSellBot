...
Любое действие, рассуждение или решение должно опираться на **документацию проекта** и **правила из памяти (`memory/`)**. Прежде чем сделать вывод или выполнить действие:
- Свериться с `docs/` — описание архитектуры, логирования, деплоя, терминология
- Свериться с `CLAUDE.md` — правила разработки, тестирования, ветвления
- Свериться с `memory/` — накопленные решения, предпочтения пользователя, feedback
- Если документация и правила не описывают ситуацию — спросить пользователя

**Non-goal:** Выполнять только задания, непосредственно данные пользователем. Любые дополнительные действия (исправления, пересборки, пуш, деплой) — только после явного разрешения.

---

## Терминология

В проекте используются следующие ключевые термины (обязательны к соблюдению в коде, документации и общении):

| Термин | Описание |
|--------|----------|
| **Super-admin** / **Супер-админ** | Владелец бота. Указывается через `ADMIN_TELEGRAM_IDS`. Глобально: **управление подписками организаторов (ТГ+ВК)**, статистика, рассылка, здоровье. **НЕ управляет каналами, мероприятиями и площадками организаторов.** Эксклюзивен — не получает организаторский раздел. |
| **Организатор без группы** | Клиент с подпиской (на пользователя или канал), без площадки. Создаёт мероприятия, продаёт билеты, проверяет на входе. Работает через Mini App. **Только организатор создаёт мероприятия.** |
| **Организатор с группой** | Организатор + подключённая площадка (**TG-канал** с активной подпиской ИЛИ **VK-группа**). Бесшовный переход из «без группы» (добавил площадку → стал с группой). Групп может быть несколько. |
| **Пользователь** | Посетитель мероприятий. **Покупает билеты**, **получает пригласительные**, на мероприятии **предъявляет код/QR на входе**. Минимальный ЛК (билеты). Стать организатором — через подписку. |
| **Канал (Channel)** | Telegram-канал, в который добавлен бот. Площадка организатора для публикации анонсов. Мероприятие привязано к каналу ИЛИ к организатору (owner_user_id). |

> **Матрица ролей:** `docs/roles-matrix.md`. Super-admin ≠ Организатор (без/с группой) ≠ Пользователь. Суперадмин управляет только глобальными подписками организаторов; мероприятия и площадки — зона организатора.
...
| 🎛 **Управление** | Админу — панель управления мероприятием в ЛС (статистика, вкл/откл, репост). Обычным пользователям — alert «❌ Доступно только администратору» |
...
Админ-меню строится через inline-кнопки в зависимости от роли. Все кнопки выполняют действие **напрямую**:

- **Inline-результат** — статистика, списки каналов, здоровье и т.д. показываются в том же сообщении
- **FSM-ввод** — кнопки, требующие параметр (подписка, инфо о канале и т.д.), открывают диалог: пишите значение прямо в чат, бот выполняет действие. Отмена: `/cancel`
- **FSM-мастер** — создание мероприятия и рассылка запускают пошаговый мастер

**Super-admin видны все кнопки. Channel admin — только общие.**

| Категория | Кнопка | Что делает | Ввод | Кому видна |
|-----------|--------|------------|------|------------|
| Статистика | 📊 Общая статистика | Сводка по всем каналам/мероприятиям/билетам | — | Super-admin |
| | 🔍 Проверить подписки | Отключает просроченные подписки | — | Super-admin |
| Каналы | 📋 Список каналов | Все каналы со статусом подписки | — | Super-admin |
| | 📢 Мои каналы | Ваши каналы и статус подписки | — | Все админы |
| | ℹ️ Инфо о канале | Детальная информация о канале | @channel (FSM) | Super-admin |
| | 👥 Инфо о пользователе | Информация о пользователе | user_id (FSM) | Super-admin |
| Подписки | 🟢 Подписать | Активировать подписку | @channel + days (FSM) | Super-admin |
| | 🔴 Отписать | Отключить подписку | @channel (FSM) | Super-admin |
| | 🔄 Сменить админа | Сменить владельца канала | @channel + new_id (FSM) | Super-admin |
| Действия | 🎫 Создать мероприятие | Пошаговый мастер создания | FSM-мастер | Все админы |
| | 📋 Мои мероприятия | Список всех мероприятий канала | — | Все админы |
| | 🔍 Проверить билет | Ввод кода билета для валидации на входе | код (FSM) | Все админы |
| | 🔄 Репост анонсов | Перепостить все анонсы в канал | — | Все админы |
| | 📢 Рассылка | Пошаговый мастер рассылки | FSM-мастер | Super-admin |
| | 🩺 Здоровье | Статус бота, БД, username | — | Super-admin |
| | ✅ Отменить билет | Отмена билета | ticket_id (FSM) | Super-admin |
...
### 🧪 Сначала тесты, потом код (TDD)

**Любая фича начинается с написания тестов.** Сначала тесты (красные — падают, т.к. код ещё не существует), затем реализация (зелёные). Правило распространяется на любые изменения логики (новые методы сервисов, эндпоинты, модели).

**Порядок:**
1. Написать тесты, описывающие ожидаемое поведение
2. Убедиться, что они падают (красные) — подтверждение, что тест реально проверяет новое
3. Реализовать код
4. Убедиться, что все тесты проходят (зелёные) + существующие не сломаны
