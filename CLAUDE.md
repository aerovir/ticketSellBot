...
| Термин | Описание |
|--------|----------|
| **Super-admin** / **Супер-админ** | Администратор бота. Указывается через `ADMIN_TELEGRAM_IDS`. Имеет полный глобальный доступ, не привязан к каналу. |
| **Admin** / **Админ канала** | Владелец или администратор одного или нескольких каналов. Управляет только своими каналами. |
...
> **Super-admin ≠ Admin.** Super-admin управляет ботом глобально. Admin управляет своим каналом. Обычный пользователь — покупает билеты.
